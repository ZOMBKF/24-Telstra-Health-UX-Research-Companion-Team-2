...
//===== Edge Case box =====
// This is the information that the AI is expected to provide for each edge case in its JSON output. The box renders a table of these items.
...
function BoxNode({ id, data, selected, type }: NodeProps) {
  ...
  // checks whether this is the Edge case Box.
  const isEdgeCase = boxType === "edge_case";
  ...
  // Only reads the AI output when this is an Edge Case Box and the AI has returned some text.
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 flex-1 min-w-0">
            {/* Show the custom warning icon for the Edge Case Box */}
            {isEdgeCase ? (
              <span className="relative flex h-5 w-5 flex-shrink-0 items-center justify-center rounded bg-pink-300">
        ...
        <div className="px-3 py-2 flex-1 min-h-0 overflow-y-auto">
          ...
          {!isInputBox && !isCartoon && !isSlides && !isCode && !isAgent && !isEdgeCase && (
            <div className="min-h-[80px]">
              {isRunning && (
